...
def getMovieInfo(name, url):
    ...
    while c:
        ...
        p="122.192.66.50"
        try:
            html = requests.get(url,headers=headers,proxies={'https':p},timeout=5).content.decode("utf-8")
            c=0
        except:
...
if __name__ == '__main__':
    strCat = lambda x, y: x + "," + y
    output = open("douban_movie_info_by_year.txt", "w")
    notdefined = open("not.txt", "w")
    with open("2016_douban_movies.txt", "r") as f:
        count = 0
        while True:
            count = count + 1
            if count % 100 == 0:
            ...
            output.write("\n")
            if (dm["name"] == "NotDefined"):
                notdefined.write(name + '\n')
            delay = random.random() * 4 + 2
            print(delay, 's')
            time.sleep(delay)
